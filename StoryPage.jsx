...
import { postToMaster } from "../services/api";
import EditableStoryInfo from "../components/EditableStoryInfo";

const StoryPage = ({ currentProject, setCurrentProject }) => {
  const [formData, setFormData] = useState({
    topic: currentProject?.story?.topic || "",
    description: currentProject?.story?.description || "",
  ...
  useEffect(() => {
    // Only update formData if the project changes and a story already exists
    if (currentProject && currentProject.story) {
      ...
      // Clear the form if no story exists for the current project
      setFormData({
        topic: "",
        description: "",
        style: "Cartoon",
  ...
  const handleGenerateStory = async (e) => {
    ...
    try {
      const response = await postToMaster({
        ...formData,
